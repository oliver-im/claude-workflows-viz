 ...
 * checked-in `examples/level-N/` corpus and the `workflow-readability`
 * before/after images stay in sync with the code. Requires a fresh `dist/cli.js`
 * (`npm run build`).
 */
const root = join(dirname(fileURLToPath(import.meta.url)), "..");
const cli = join(root, "dist/cli.js");

const dirs = ["examples/level-1", "skills/workflow-readability/example"];
const jobs = [];
