...
describe("flattenTopology — parallel fan-out", () => {
  it("emits body + barrier with the join edge; predecessor enters at the body head", () => {
    ...
    expect(ir.nodes.map((n) => n.kind)).toEqual(["agent", "agent", "barrier"]);
    expect(ir.nodes[1].mult).toEqual({ kind: "named", names: ["x", "y"] });
    expect(ir.nodes[2].label).toBe("");
    // join first (inside the structure), then the same-band entry edge
    expect(fmt(ir.edges)).toEqual(["n1->n2", "n0->n1"]);
  });
  ...
  it("places the barrier in the band of the last contributing node; lane-internal succession keeps the same-band rule", () => {
    ...
    expect(ir.nodes.map((n) => n.band)).toEqual([0, 1, 1]); // barrier follows the lane terminal
    expect(fmt(ir.edges)).toEqual(["n1->n2"]); // a→b crosses bands implicitly → no edge
  });
});
...
describe("flattenTopology — loop", () => {
  ...
  it("bands the decision with the last body node, spanning bands like hunt-bugs", () => {
    ...
    expect(ir.nodes.map((n) => n.kind)).toEqual(["agent", "agent", "barrier", "decision"]);
    expect(ir.nodes.map((n) => n.band)).toEqual([0, 1, 1, 1]);
    expect(fmt(ir.edges)).toEqual(["n1->n2", "n2->n3"]); // find→fanout stays implicit cross-band
    expect(fmt(ir.loops)).toEqual(["n3->n0:yes"]); // back arc crosses bands 1 → 0
  });
...
describe("per-example graphs", () => {
  it("review-pr: slices → review ×3(expanded) ⇒ verify ×?(findings) ⇒ barrier; only the stage chain crosses bands", () => {
    ...
    // the fan-out join, then the explicit stage chain crossing bands 1 → 2
    expect(fmt(ir.edges)).toEqual(["n2->n3", "n1->n2"]);
    expect(ir.loops).toEqual([]);
  });
  ...
  it("summarize-codebase: list, fan out ×?(modules), join — and nothing else (cross-band hops stay implicit)", () => {
    ...
    expect(ir.nodes.map((n) => n.band)).toEqual([0, 1, 1, 2]);
    expect(ir.nodes[1].label).toBe("read:${m}");
    expect(ir.nodes[1].mult).toEqual({ kind: "unknown", hint: "modules.modules" });
    expect(fmt(ir.edges)).toEqual(["n1->n2"]); // exactly one edge: the fan-out join
    expect(ir.loops).toEqual([]);
  });
  ...
  it("name-the-feature: gen fan-out expanded gen:* joins; filter and shortlist stack bandwise", () => {
    ...
    expect(ir.nodes.map((n) => n.kind)).toEqual(["agent", "barrier", "agent", "agent"]);
    expect(ir.nodes.map((n) => n.band)).toEqual([0, 0, 1, 2]);
  ...
  it("choose-approach: draft ×4, nested while>for tournament (two decisions, two back arcs), write-up on the 'no' exit", () => {
    ...
    expect(ir.nodes.map((n) => n.band)).toEqual([0, 0, 1, 1, 1, 3]);
    expect(ir.nodes[0].mult).toEqual({
      kind: "named",
      names: ["draft:simplest", "draft:most scalable", "draft:least risky", "draft:fastest to ship"],
    });
    expect(ir.nodes[2].label).toBe("match:${i / 2}");
    expect(ir.nodes[3].label).toBe("i < bracket.length");
    expect(ir.nodes[4].label).toBe("bracket.length > 1");
    expect(fmt(ir.edges)).toEqual(["n0->n1", "n2->n3", "n3->n4:no", "n4->n5:no"]);
    expect(fmt(ir.loops)).toEqual(["n3->n2:yes", "n4->n2:yes"]);
  });
  ...
  it("hunt-bugs: find → verify fan-out → join → decision, looping back across bands; no exit (the loop ends the workflow)", () => {
    ...
    expect(ir.nodes[3].label.startsWith("dryRounds < 2 &&")).toBe(true);
    expect(ir.nodes[3].label.endsWith("…")).toBe(true); // verbatim, COND_MAX-truncated
    expect(fmt(ir.edges)).toEqual(["n1->n2", "n2->n3"]);
    expect(fmt(ir.loops)).toEqual(["n3->n0:yes"]); // the back arc spans bands 1 → 0
  });
