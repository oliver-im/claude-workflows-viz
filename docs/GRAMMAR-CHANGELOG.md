...
| [`cc-2.1.280`](../spec/upstream/2026-09-22-cc-2.1.280/) (2026-09-22) | 20606 | `596cf079d0a01dfd…` | Two minifier identifier renames inside the prose's interpolations (`${PP}`→`${tN}` in the `workflow()` group-name sentence, `${dC}`→`${Fk}` in the per-call item cap). Both are 2 characters, so the byte count is unchanged and only the hash moved; the input schema is untouched. No vocabulary change, so it stays level 2. |

`RECOGNIZER_LEVEL_CC` tracks the newest baseline (`2.1.280`), not the version the
